...
router.get('/vast3_xml_generation', function(req, res, next){
	...
	// vast3_xml.node('VAST').attr({version:'3.0' , 'xmlns:xsi':'http://www.w3.org/2001/XMLSchema-instance' , 'xsi:noNamespaceSchemaLocation':'vast.xsd'});
  	// vast3_xml.node('root')
   ...
   //  	.node('sibling', 'with content!');
	...
	vast3_xml = new libxmljs.Document();
...
function AdTitle(xmlDoc){
	...
	temp_vast_xml = temp_vast_xml.node('AdTitle', AdTitle );
	temp_vast_xml = temp_vast_xml.parent();
...
function Description(xmlDoc){
	...
	temp_vast_xml = temp_vast_xml.node('Description', Description );
	temp_vast_xml = temp_vast_xml.parent();
...
function Error(xmlDoc){
	...
	temp_vast_xml = temp_vast_xml.node('Error', Error);
	temp_vast_xml = temp_vast_xml.parent();
...
function Impression(xmlDoc){
	...
	for (var i=0; i<ImpressionArr.length; i++){
		...
		temp_vast_xml = temp_vast_xml.node('Impression' , ImpressionArr_text ).attr({'id' : ImpressionArr_id });
		temp_vast_xml = temp_vast_xml.parent();
		...
		for (var i=0; i<TrackingEventsArr.length; i++ ){
			if (TrackingEventsArr[i].name() == "Tracking"){
				var TrackingEvents_event = TrackingEventsArr[i].attr('event').value();
		   		...
		   		temp_vast_xml = temp_vast_xml.node('Tracking' , TrackingEvents_text ).attr({'event' : TrackingEvents_event });
		   		temp_vast_xml = temp_vast_xml.parent();
...
function ClickThrough(xmlDoc){
	...
	temp_vast_xml = temp_vast_xml.node('ClickTracking',ClickThrough);
	temp_vast_xml = temp_vast_xml.parent();
...
function MediaFile(xmlDoc){
	...
	temp_vast_xml = temp_vast_xml.node('MediaFile' , MediaFile_text ).attr({
	...
	temp_vast_xml = temp_vast_xml.parent(); //MediaFile
}
